...
# This file will be used for Julia version $(VERSION) or higher if there is no other latest builtins files.
...
""")
        ...
        elseif f === Core._apply ||
               @static isdefined(Core, :_call_latest) ? f === Core._call_latest : false ||
               @static isdefined(Core, :_apply_latest) ? f === Core._apply_latest : false
...
"""
    $head f === $fstr
            if nargs == 3
                return Some{Any}($fstr(@lookup(frame, args[2]), @lookup(frame, args[3]), @lookup(frame, args[4])))
            else
                return Some{Any}($fstr(getargs(args, frame)...))
            end
"""
)
            continue
...
generate_builtins(normpath(@__DIR__, "..", "src", "builtins", string(VERSION)))
